...
<tr>
    ...
    %ifvar ../mode equals('view')%
	    ...
	    <INPUT TYPE="radio" NAME="ssl" id="ssl" VALUE="true" style="visibility: hidden;"   %ifvar ssl equals('true')%CHECKED%endif%>
    %else%
        <INPUT TYPE="radio" NAME="ssl" id="ssl" VALUE="false" %ifvar ssl equals('true')%%else%CHECKED%endif% onclick="toggleSSL(false, document.properties1);"><label for="ssl">HTTP</label>
        <INPUT TYPE="radio" NAME="ssl" id="sslRIGon" VALUE="true"   %ifvar ssl equals('true')%CHECKED%endif%  onclick="toggleSSL(true, document.properties1);"><label for="sslRIGon">HTTPS</label>
    %endif%
    </td>
</tr>
...
<tr>
     <td class="oddrow"><label for="keepAliveTimeout">Keep Alive Timeout</label></td>
     <td class="%ifvar ../mode equals('view')%oddrowdata-l%else%oddrow-l%endif%">
        %ifvar ../mode equals('view')%
		  	%value keepAliveTimeout encode(html)%
        %else%
          %ifvar keepAliveTimeout%
		    <input type="text" name="keepAliveTimeout" id="keepAliveTimeout" value="%value keepAliveTimeout encode(htmlattr)%">
          %else%
		    <input type="text" name="keepAliveTimeout" id="keepAliveTimeout" value="20000">
          %endif%
        %endif%
     </td>
</tr>
