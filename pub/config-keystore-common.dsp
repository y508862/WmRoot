			...
			<td class="oddrow">Use JSSE</td>
		    	...
		    	%ifvar mode equals('view')%
		        	%switch useJSSE%
		             ...
		             %endswitch%
		         %else%
				<input type="radio" id="useJSSE1" name="useJSSE" value="yes" %switch useJSSE%
		            	    %case 'yes'%checked
		            	    %case 'true'%checked
							%case%checked
		             %endswitch%><label for="useJSSE1">Yes</label></input>
				<input type="radio" id="useJSSE2" name="useJSSE" value="no" %switch useJSSE%
		            	    %case 'no'%checked
		            	    %case 'false'%checked
		             %endswitch%><label for="useJSSE2">No</label></input>		         
		          %endif%
		       </td>
	</tr>
	<tr>
		<td class="oddrow"><label for="useJSSE1">Keystore Alias</label></td>
		<td class="%ifvar ../mode equals('view')%oddrowdata-l%else%oddrow-l%endif%">
			%ifvar mode equals('view')%
				 %value keyStore encode(html)%
			%else%
				 <SELECT NAME="keyStore" id="keystore" class="listbox" onchange="changeval(enclosedFormName.value)"></SELECT>
			%endif%
		</td>
	 </tr>
	 
	 <tr>
		<td class="evenrow" valign="top"><label for="alias">Key Alias</label></td>
		<td class="%ifvar ../mode equals('view')%evenrowdata-l%else%evenrow-l%endif%">
			%ifvar mode equals('view')%
				 %value alias encode(html)%
			%else%
				<select name="alias" id="alias" class="listbox"></select>
			%endif%
		</td>
	 </tr>
	 
	 <tr>
		<td class="oddrow"><label for="trustStore">Truststore Alias</label></td>
		<td class="%ifvar ../mode equals('view')%oddrowdata-l%else%oddrow-l%endif%">
			%ifvar mode equals('view')%
...
%else%
    ...
    <tr>
     ...
     <tr>
		<td class="evenrow"><label for="keyStore">Keystore Alias</label></td>
        <td class="%ifvar ../mode equals('view')%oddrowdata-l%else%oddrow-l%endif%">
            %ifvar mode equals('view')%
                 %value keyStore encode(html)%
            %else%
				 <SELECT id="keyStore" NAME="keyStore" class="listbox" onchange="changeval(enclosedFormName.value)"></SELECT>
            %endif%
        </td>
     </tr>
     
     <tr>
		<td class="oddrow" valign="top"><label for="alias">Key Alias</label></td>
        <td class="%ifvar ../mode equals('view')%evenrowdata-l%else%evenrow-l%endif%">
            %ifvar mode equals('view')%
                 %value alias encode(html)%
            %else%
				<select id="alias" name="alias" class="listbox"></select>
            %endif%
        </td>
     </tr>
