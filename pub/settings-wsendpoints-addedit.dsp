    ...
    */
    ...
    function confirmEdit()
    {
        ...
        %else%
			...
			if(!processKrbTransportOnSubmit()){
                return false;
            }
			
			if(!validateKRBSettingsTransport()){
			    return false;
            }
...
       		
    ...
    function controlKerberosAuthTypeDropdown()
	{   
		var authTypedropdown = document.getElementById("authType");
		if(authTypedropdown != null && authTypedropdown.options.length == 3)
		{
		 var option = document.createElement("OPTION");
 		 option.text = "Kerberos";
 		 option.value = "kerberos"; 
	...
	function selectKerberosAuthType()
	{
	 var jaasContext = document.getElementById("krbJaasContextTransport").value ;
	 
	 if(null != jaasContext && jaasContext != "")
	 {
	   var authTypedropdown = document.getElementById("authType");
	   if(authTypedropdown.options.length == 4 && authTypedropdown.options[3].value == "kerberos")
	   {
	...
	function processKrbTransportOnSubmit(){
        var clientPwd = trim(document.getElementById("krbClientPasswordTransport").value);
        var clientRePwd = trim(document.getElementById("retype_krbClientPasswordTransport").value);
        
        var pwdConfirm = confirmPassword(clientPwd, clientRePwd, "Kerberos Transport Client");
        return pwdConfirm;
    }
	
	function validateKRBSettingsTransport(){
	var authTypedropdown = document.getElementById("authType");
	if(null != authTypedropdown && authTypedropdown.options.length==4 && authTypedropdown.options[3].value == 'kerberos' && authTypedropdown.options[3].selected == true)
	{
	   if(!validateField("JAAS Context", document.getElementById("krbJaasContextTransport").value)){
        return false;
    }
    if(!validateField("Principal", document.getElementById("krbClientPrincipalTransport").value)){
        return false;
    }
    if(!validateField("Principal Password", document.getElementById("krbClientPasswordTransport").value)){
        return false;
    }
	if(!validateField("Retype Principal Password", document.getElementById("retype_krbClientPasswordTransport").value)){
        return false;
    }
	if(!validateField("Service Principal Name", document.getElementById("krbServicePrincipalTransport").value)){
        return false;
       }  
	}
  ...
  function validateField(field,valueToValidate){
	...
	{
	  alert(""+field+" in Kerberos Transport Properties must be provided to use kerberos authentication at transport level.") 
	  return false;
	}
    return true;
  }
                    ...
                    %scope transportInfo%
                        ...
                        %ifvar transportType equals('HTTPS')%
                            ...
                            <script>
                                ...
                                displayRows("consumerHTTPSDiv");
                                document.getElementById('httpTransportHeaderOpt').innerHTML = "&nbsp;(optional)";
								controlKerberosAuthTypeDropdown();
								selectKerberosAuthType();
                            </script>
                        %endif%
                        %ifvar transportType equals('HTTP')%
                            %include wsendpoints-HTTP-props.dsp%
