      ...
      <tr>
	      <td class="oddrow"><label for="port">Port</label></td>
	       <td class="%ifvar ../mode equals('view')%oddrowdata-l%else%oddrow-l%endif%">
	         ...
	         %else%
		         <input id="port" name="port" value="%value port encode(htmlattr)%">
	         %endif%
	       </td>
      </tr>
      <tr>
         ...
         <td class="%ifvar ../mode equals('view')%evenrowdata-l%else%evenrow-l%endif%">
           %ifvar listenerKey -notempty%
			 ...
			 %else%
				<input id="portAlias" name="portAlias" value="%value portAlias encode(htmlattr)%" size="60" disabled>
			 %endif%
           %else%
...
%else%
	...
	<td class="%ifvar ../mode equals('view')%evenrowdata-l%else%evenrow-l%endif%">
	 ...
	 %else%
	   %ifvar maxQueue%
	     <input id="maxQueue" name="maxQueue" value="%value maxQueue encode(htmlattr)%">
	   %else%
	     <input id="maxQueue" name="maxQueue" value="200">
	   %endif%
	 %endif%
	</td>
   </tr>
